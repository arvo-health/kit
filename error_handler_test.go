...
func TestErrorHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			inputError:     kit.NewResponseError(http.StatusBadRequest, errors.New("bad request")),
		...
		},
		{
			...
			inputError: kit.NewResponseError(http.StatusUnprocessableEntity,
				kit.NewValidationErrors("validation error", "field1 is required")),
		...
		{
			name: "ResponseError with custom Error passed to handler",
			inputError: kit.NewResponseError(http.StatusConflict,
				kit.NewErrorf("SOME_CODE", "any %s message", "error")),
